<script lang="ts">
  ...
  $effect(() => {
    Cookies.set('chosenEdition', editions.findIndex((edition) => edition.id === chosenEdition.id).toString())
  })
  $effect(() => {
...
<main>
  <Header bind:chosenEdition bind:enableMajors bind:enableMinors bind:enableDrops bind:enableEvents bind:isVertical />
  <Timeline edition={chosenEdition} types={getEnabledTypes()} {isVertical} />
  <Footer />
  {#each hoveredVersions as version (version.title)}
    <Tooltip {version} />
  {/each}
  <Book />
</main>
