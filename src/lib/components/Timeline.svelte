<script lang="ts">
  ...
  let { edition, types, isVertical }: { edition: Edition; types: VersionType[]; isVertical: boolean } = $props()
  let years = $derived(calculateYears(edition))
  let min_year = $derived(Math.min(...Object.keys(years).map(Number)))
  let this_year = new Date().getFullYear()
  let all_years = $derived(Array.from({ length: this_year - min_year + 1 }, (_, i) => min_year + i))
</script>

<div class="timeline">
  <div class="ruler-base"></div>
  <div class="years">
    {#each all_years as year (year)}
      <Year {edition} {year} {isVertical} versions={year in years ? years[year].filter((pair) => types.includes(pair.version.type)).sort((a, b) => new Date(a.version.date).getTime() - new Date(b.version.date).getTime()) : []} first={year === min_year} last={year >= this_year} />
    {/each}
